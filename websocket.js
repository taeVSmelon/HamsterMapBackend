...
wss.on('connection', (ws, req) => {
  ...
  if (event === 'raid' && playerId) {
    if (raidBoss.active) {
      ...
      ws.send(JSON.stringify({ e: 'RS', b: raidBoss.bossPrefabName, mH: raidBoss.maxHealth, h: raidBoss.health }));
    }
    else ws.close();
  ...
  ws.on('message', (message) => {
    ...
    switch (signal) {
      case 'TD':
        ...
        console.log(`${userId} dealt ${damage} damage. Boss HP: ${raidBoss.health}`);
...
app.post('/notify/start-raid', (req, res) => {
  ...
  if (!bossPrefabName || !maxHealth) return res.status(400).json({ error: 'Missing data' });
